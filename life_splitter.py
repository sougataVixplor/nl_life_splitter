...
def get_life_name(text,prev_li_num=None):
...
def get_life_toc(pdf_path):
    ...
    for page_num in range(len(doc)):
        ...
        if len(life_toc)!=0:
          prev_li_num=int(life_toc[-1]["topic"].split("-")[1])
          l_name=get_life_name(text,life_toc)
        else:
          l_name=get_life_name(text)
        ...
        elif l_name is not None and life_toc[-1]["topic"]!=l_name and l_num>int(life_toc[-1]["topic"].split("-")[1]):
          life_toc.append({
              "topic":l_name,
              "page":page_num
          })
